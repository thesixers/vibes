...
const ArtistPage = () => {
  ...
  if (!artistData) return <div className="p-10 text-white/50 font-mono text-sm">LOADING ASSET...</div>;

  const { artist, songs, albums, topSongs } = artistData;

  // Use the image of the first album as a fallback cover if artist has no image
  const artistImage = artist.images?.[0]?.url || albums[0]?.images?.[0]?.url || null;
    ...
    <div className="flex-1 overflow-y-auto custom-scrollbar bg-black pb-32">
      ...
      <div className="relative w-full h-[50vh] min-h-[400px]">
        ...
        <div className="absolute inset-0">
          {artistImage ? (
            <img src={artistImage} alt={artist.name} className="w-full h-full object-cover opacity-60 mask-gradient-bottom" />
          ) : (
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 md:p-12 z-10 flex flex-col justify-end items-start gap-6">
            ...
            <div className="flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-sm rounded-full border border-white/5">
                ...
                <span className="text-xs font-bold tracking-widest text-white uppercase">Artist</span>
            </div>

            {/* Name - Huge & Tight */}
            <h1 className="text-6xl md:text-9xl font-black text-white tracking-tighter leading-none mix-blend-overlay opacity-90">
                {artist.name}
            </h1>
      ...
      <div className="p-6 md:p-12 grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-12">
        ...
        <div>
            <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-3">
                Top Tracks
                <span className="text-sm font-normal text-white/40">Most played</span>
            </h2>

            <div className="space-y-1">
                {topSongs.map((song, index) => {
                    const isCurrent = currentTrack?.id === song.id;
                    const img = song.album?.images?.[2]?.url || song.album?.images?.[0]?.url;
                    ...
                    return (
                        <div
                            ...
                            `}
                        >
                            <span className="w-6 text-center text-white/30 font-mono text-sm group-hover:hidden">{index + 1}</span>
                            <span className="w-6 hidden group-hover:flex justify-center text-white"><Play size={14} fill="white"/></span>

                            {img ? (
                                <img src={img} alt="" className="w-12 h-12 rounded bg-white/5 object-cover" />
                            ) : (
                                <div className="w-12 h-12 rounded bg-white/5 flex items-center justify-center"><Music size={16} className="text-white/20"/></div>
                            )}

                            <div className="flex-1 min-w-0">
                                <h4 className={`font-medium truncate text-base ${isCurrent ? 'text-purple-electric' : 'text-white'}`}>
                                    {song.title}
                                </h4>
                                <p className="text-xs text-white/40 truncate">{song.album?.title}</p>
                            </div>
                        ...
                        {albums.slice(0, 4).map(album => (
                            ...
                            >
                                <div className="aspect-square rounded-xl overflow-hidden bg-white/5 relative">
                                    {album.images?.[0]?.url ? (
                                        <img src={album.images[0].url} alt={album.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                                    ) : (
                                        <div className="w-full h-full flex items-center justify-center text-white/20"><Disc/></div>
                                    )}
                                </div>
                                <div>
                                    <h3 className="font-bold text-white text-sm truncate group-hover:underline decoration-white/30 underline-offset-4">{album.title}</h3>
                                    <p className="text-xs text-white/40">{new Date(album.release_date).getFullYear()} • Album</p>
                                </div>
                            </div>
                        ))}
                    </div>
